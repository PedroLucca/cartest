  ...
  <form @submit.prevent="submit">
    <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
      <div class="flex flex-col space-y-1">
        ...
        <SectionTitle class="text-xs text-gray-600">Informações essenciais para edição de Funcionários no sistema.
        </SectionTitle>
      </div>
      ...
      <div class="mt-10 grid grid-cols-1 gap-6 max-md:grid-cols-1">
        ...
        <div>
          <span class="file-input border w-full p-1 text:sm lg:col-span-1">
            <input @change="attachAvatar" type="file" />
            <span class="button text-gray-400">Anexar</span>	
            <label class="label" data-js-label>{{ form.anexo?.name || 'Nenhum arquivo selecionado' }}</label>
          </span>
        </div>
        <span v-if="form.anexo?.name" class="inline-flex rounded-full bg-primary text-center px-4 text-xs font-semibold loading-5 text-white w-fit">Arquivo anexado</span>
        <img :src="route('download.files', props?.ConfigFuncionarios?.anexo ? props?.ConfigFuncionarios?.anexo : '')"
     @error="event => event.target.src = '/images/profile_default.png'"
     alt=""
     style="width: 150px"/>
        <div>
        ...
        <div>
        <span class="p-float-label">
          <InputText v-model="form.cpf" id="cpf" type="text" class="w-full" required maxlength="14" />
      ...
      <div>
        <span class="p-float-label">
          <InputText v-model="form.cargo" id="cargo" type="text" class="w-full" required maxlength="50" />
      ...
      <div>
        <span class="p-float-label">
          <InputText v-model="form.email" id="email" type="email" class="w-full" required maxlength="100" />
      ...
      <div>
        <span class="p-float-label">
          <InputText v-model="form.telefone" id="telefone" type="tel" class="w-full" required maxlength="15" />
...
const form = useForm({
  ...
  cargo: $propsPage?.value.ConfigFuncionarios?.cargo,
  
  email: $propsPage?.value.ConfigFuncionarios?.email,

  anexo: $propsPage?.value.ConfigFuncionarios?.anexo,

  telefone: $propsPage?.value.ConfigFuncionarios?.telefone,

  status: { value: $propsPage?.value.ConfigFuncionarios?.status},
