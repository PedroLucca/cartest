<template>
  <div class="bg-white px-4 shadow-lg rounded-xl">
    ...
    <AchievementCard
      ...
      :RotaCadastro="route('form.store.ConfigVeiculos')"
        ...
        <div class="space-x-6 flex ml-4 grid grid-cols-6 gap-6 max-md:grid-cols-1">
          ...
          <div class="flex items-center space-x-2">
            <Checkbox
              @change="toggleColumns"
              :binary="true"
              v-model="formColumns['columns']['tipo_veiculo']"
            />
    ...
    <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
      <div class="inline-block min-w-full py-2 align-middle">
        <h2 class="text-xl font-semibold fontInter text-center py-5"><b>Veículos</b></h2>
        <div
          ...
          <table class="min-w-full divide-y divide-gray-300">
            <thead class="mb-24">
              <tr class="text-gray-500 font-bold select-none" @click="setParams">
                ...
                <th
                  v-if="formColumns.columns.tipo_veiculo"
                  scope="col"
                  class="px-4 text-sm cursor-pointer text-center border-r group"
                  @click="
                    orderBy = {
                      column: 'tipo_veiculo',
                      sorting: sortTable(sortVal.tipo_veiculo_id)
                        ? (sortVal.tipo_veiculo_id = 1)
                        : (sortVal.tipo_veiculo_id = 0),
                    }
                  "
                >
                  <div class="flex">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              ...
              >
                ...
                >
                  <div class="flex items-center">
                    <div>
                      <div class="font-medium text-gray-900">{{ getTipoVeiculo(data?.tipo_veiculo_id) }}</div>
                    </div>
                  </div>
                </td>
                <td
                ...
                >
                  <Actions
                    :routeDel="route('delete.ConfigVeiculos', { id: data?.token })"
...
function getTipoVeiculo(id) {
    const tipo = $propsPage?.value?.tipos_veiculos.find((t) => t.id === id);
...
const formColumns = useForm({
  ...
  columns: {
    ...
    tipo_veiculo: validateColumnsVisibility("tipo_veiculo"),
...
const form2 = useForm({
  ...
  tipo_veiculo_id: props.Filtros?.tipo_veiculo_id || null,
